...
using me.cqp.luohuaming.PalPal.Core;
using me.cqp.luohuaming.PalPal.Core.Domain;
using me.cqp.luohuaming.PalPal.Sdk.Cqp;
using me.cqp.luohuaming.PalPal.Sdk.Cqp.Enum;
using me.cqp.luohuaming.PalPal.Sdk.Cqp.EventArgs;
using me.cqp.luohuaming.PalPal.Sdk.Cqp.Interface;
using me.cqp.luohuaming.PalPal.Sdk.Cqp.Expand;
using me.cqp.luohuaming.PalPal.Sdk.Cqp.Model;
using Unity;
using Unity.Injection;

namespace me.cqp.luohuaming.PalPal.App.Export
{
	/// <summary>	
	/// 表示酷Q事件导出的类	
	/// </summary>	
	...
	{	
		...
		{	
			return "9,me.cqp.luohuaming.PalPal";	
		}	
		...
		{	
			...
			Type appDataType = typeof (AppData);	
			// 注册一个 CQApi 实例	
			AppInfo appInfo = new AppInfo ("me.cqp.luohuaming.PalPal", 1, 9, "帕鲁帕鲁", "1.0.0", 1, "落花茗", "帕鲁服务器控制", authCode);	
			appDataType.GetRuntimeProperty ("CQApi").GetSetMethod (true).Invoke (null, new object[] { new CQApi (appInfo) });	
			AppData.UnityContainer.RegisterInstance<CQApi> ("me.cqp.luohuaming.PalPal", AppData.CQApi);	
			// 向容器注册一个 CQLog 实例	
			appDataType.GetRuntimeProperty ("CQLog").GetSetMethod (true).Invoke (null, new object[] { new CQLog (authCode) });	
			AppData.UnityContainer.RegisterInstance<CQLog> ("me.cqp.luohuaming.PalPal", AppData.CQLog);	
		...
		{	
			...
			{	
				CQGroupMessageEventArgs args = new CQGroupMessageEventArgs (AppData.CQApi, AppData.CQLog, 2, 2, "群消息处理", "_eventGroupMsg", 30000, subType, msgId, fromGroup, fromQQ, "", msg.ToString(CQApi.DefaultEncoding), false);	
